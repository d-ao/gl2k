    ...
    initMap = function () {
        ...
        google.maps.event.addListener(map, 'zoom_changed', function () {
            ...
            if (zoomLevel <= desktopShowCom && win_width > mobileBreakpoint) {
                ...
                // console.log('aktiviere kml layer bundesland deaktiviere gemeinden');
                ...
                // console.log('aktiviere kml layer gemeinden deaktiviere bundesland');
        ...
        // TEST für KML Layer
        // KML LAYER FUNKTIONIERNE nUR WENN SIE VON GOOLGE GELADEN WERDEN KÖNNEN!
        // var kml_url = dirBase + statesKml;
        var kml_url = dirBase + '/data/not_there.kml';
        console.log('kml_url: ' + kml_url);
        var ctaLayer = new google.maps.KmlLayer({
        ...
        // var geo_json_url = dirBase +'/data/oesterreich.json';
        // console.log('geo_json_url: ' + geo_json_url);
        // map.data.loadGeoJson(geo_json_url);

        var geo_json_url2 = dirBase + '/data/bundeslaender_und_gemeinden.json';
        console.log('geo_json_url2: ' + geo_json_url2);
        map.data.loadGeoJson(geo_json_url2);
        ...
        map.data.setStyle(function (feature) {
            var rtype = feature.getProperty('rtype');


            var depth = rtype === 'bundesland' ? -10 : -20;
            var fColor = rtype === 'bundesland' ? '#55ff55' : '#ffffff';
            var fOpacity = rtype === 'bundesland' ? 0.3 : 0;
            var sColor = rtype === 'bundesland' ? 'grey' : 'lightgrey';
            var sOpacity = rtype === 'bundesland' ? 1 : 0.5;
            var sweight = rtype === 'bundesland' ? 3 : 1;
            ...
            try {
                for (var i = 0; i < community_data.length; i += 1) {
                    ...
                    var comMarker = new google.maps.Marker({
                        ...
                        /* ToDo: LatLng */
                        position: {
                            lat: Number(community_data[this_city].latitude[0]),
                            lng: Number(community_data[this_city].longitude[0])
                        },
                        /* ToDo: LatLng */
                        ...
                        icon: dirBase + iconCamera,
                        label: {
                            text: community_data[this_city].cmp_community + ': ' + community_data[this_city].garden_size + 'm²',
                            color: 'black'
                        }
                    });
